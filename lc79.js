...
function detectAll300Bridges(history) {
    ...
    if (recentStr.endsWith("1,1,1,1")) {
        ...
        if (lastValue === 'T') {
            id = 16; name = "Cầu Bệt Tài"; criteria = "Tài xuất hiện chiếm hơn 70%."; nextPredict = 'T'; targetBridge = "Phá bệt";
            const recentTX = tx.slice(-3);
            if (recentTX.filter(x => x === 'X').length >= 2) { isBreaking = true; criteria = "Sắp bẻ: Xuất hiện 2 Xỉu trong 3 phiên."; nextPredict = 'X'; targetBridge = "Hồi 1"; }
        } else {
            id = 17; name = "Cầu Bệt Xỉu"; criteria = "Xỉu chiếm ưu thế tuyệt đối."; nextPredict = 'X'; targetBridge = "Phá bệt";
            const recentTX = tx.slice(-3);
            if (recentTX.filter(x => x === 'T').length >= 2) { isBreaking = true; criteria = "Sắp bẻ: Xuất hiện 2 Tài trong 3 phiên."; nextPredict = 'T'; targetBridge = "Hồi 1"; }
        }
...
function analyzeReverseFromLatest(history, depth = 30) {
    ...
    // Lật ngược mảng để quét từ mới nhất về cũ nhất
    const reversedHistory = [...history].reverse().slice(0, depth);
    let tScore = 0;
    let xScore = 0;
    
    // Gắn trọng số cực lớn cho 5 phiên gần nhất
    for (let i = 0; i < reversedHistory.length; i++) {
        const item = reversedHistory[i];
        let weight = 1;
        if (i < 3) weight = 3.0; // 3 ván sát nhất
        else if (i < 8) weight = 2.0; // 5 ván tiếp theo
...
function masterLookupEngine(history) {
    const votes = { T: 0, X: 0 };
    // Tránh gọi đệ quy thuật toán 16
...
function algo16_SuperEvolutionEngine(history) {
    ...
    // Self-Cloning (Nhân bản giả lập): Tạo 2 vũ trụ song song cho ván tiếp theo
...
const ALL_ALGS = [
    ...
    { id: 'algo16_super_evolution_engine', fn: algo16_SuperEvolutionEngine } // NÂNG CẤP V9.0
];
    ...
    predict(history) {
        ...
        for (const a of this.algs) {
            try {
                ...
                // --- TRỌNG SỐ CHO CÁC THUẬT TOÁN VIP ---
                ...
                if (a.id === 'algo16_super_evolution_engine') weight *= 2.2; // SIÊU TRỌNG SỐ CHO AI NHÂN BẢN V9.0
...
function getComplexPattern(history) {
    ...
    // Sử dụng bộ nhận diện 300 cầu mới nâng cấp để phân tích tên cầu cụ thể
    const bridgeMatch = detectAll300Bridges(history);
...
app.get("/", async () => { 
    return {
        ...
        version: "9.0 CỖ MÁY NHÂN BẢN & PHÂN TÍCH ĐẢO NGƯỢC TỪ API (Siêu Bền Vững)", 
        algorithms: ALL_ALGS.length,
        pattern_recognition: "VIP Hoàn Chỉnh (Tích hợp Tra cứu toàn cục 15 thuật toán gốc và giả lập nhân bản tương lai nhánh)",
        endpoints: [
            "/api/taixiumd5/lc79",
...
const start = async () => {
    ...
    console.log("\n🚀 AI Tài Xỉu MD5 Pro V9.0 - CỖ MÁY NHÂN BẢN & ĐẢO NGƯỢC VIP Đã Khởi Động!");
    ...
    console.log(`\n🔧 Hệ thống AI VIP với ${ALL_ALGS.length} thuật toán (15 nguyên bản + 1 Cỗ Máy Nhân Bản):`);
    ALL_ALGS.forEach((alg, i) => console.log(`   ${i+1}. ${alg.id}`));
    console.log("\n🎯 CẬP NHẬT GIA TRỌNG MỚI (V9.0 VIP):");
    console.log("   • [GIỮ NGUYÊN CODE LÕI]: Bảo tồn 100% tài nguyên và 15 thuật toán cũ.");
    console.log("   • [PHÂN TÍCH ĐẢO NGƯỢC]: Soi kỹ cầu sát ván hiện tại để bắt thóp thuật toán MD5 của nhà cái.");
    console.log("   • [TRA CỨU TOÀN CỤC]: Chạy nền 15 thuật toán cũ để tìm 'sự đồng thuận' mạnh nhất.");
    console.log("   • [CỖ MÁY NHÂN BẢN]: Giả lập kết quả ván tới (T/X) để đo độ ổn định, tự động khắc chế khi API đổi thuật toán.");
};
start();